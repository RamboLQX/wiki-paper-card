...
- **Complete narrative, not an increment**: write `narrative.overview`, 3-5 claim-led `synthesis_blocks`, and only genuine `controversy_blocks`. Every paragraph follows topic sentence -> cross-paper evidence -> boundary/difference -> research implication, references its evidence-ledger IDs, and represents the complete current field state rather than the current batch alone.
...
- Give every open question and research gap a stable ID and `origin`; preserve both when updating or answering an existing item.
