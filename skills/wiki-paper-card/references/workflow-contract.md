...
publish-report.json
wiki source pages
deterministic entity stubs
optional topic pages
wiki/meta/research.md (research dashboard: open questions, gaps)
updated index and log
```

Paper cards are generated independently and concurrently. Cross-paper knowledge actions and topic pages are decided only after every card and digest in the batch passes its audits. Entity stubs are generated deterministically by the publisher from the batch digests; no agent decides them.
...
5. Writes the paper-local `paper-digest.json`, filling `analysis.datasets`, `analysis.models`, and `analysis.metrics` with public reusable artifacts only (see the digest schema).
...
For a single paper, the linker may only update an existing topic page that the paper directly connects to or answers. It cannot create a topic from one paper alone. Entity stubs are always generated by the publisher, also for a single paper.
...
4. Generates and updates entity stubs deterministically from the batch digests' `analysis.datasets`, `analysis.models`, and `analysis.metrics` lists: normalizes names, merges name variants into one page (shorter raw name as title, other spellings as aliases), creates a stub for new artifacts, and appends current source pages to the `## 引用来源` list of existing pages. Re-running the same batch is idempotent.
...
8. Rebuilds `wiki/meta/knowledge-tree.md` (domain-grouped navigation tree) and `wiki/meta/research.md` (domain-grouped dashboard: open questions and research gaps) from the current wiki state.
...
created source/entity/topic page counts
updated page counts
```
