...
- 你确认后，结果才通过确定性发布流程写回 Topic 页的「开放问题」和「研究空白与候选方向」区块，并同步研究仪表盘。若本轮归档了已解决项，框架再用一次批量 linker 刷新受影响 Topic 的综合叙述，不重新处理论文。
...
- 写回只按稳定 ID 维护开放问题和研究空白，不改写 Topic 叙事或论文对照表；过期计划会在写入前被阻断。
- mining 发布归档后才允许专用 refresh linker 更新叙述；全部受影响 Topic 合并为一次调用，失败时保留待刷新提示。
