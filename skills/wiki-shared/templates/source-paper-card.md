...
公式本体放在表格外，使用 `$$...$$`。表格只记录编号、符号含义、目的和来源。
...
验证方式
可能的失败模式
创新状态：unverified / partially checked / prior-art checked
```

## 提取的实体与概念

**公共实体（数据集/基准/模型家族/指标，进入 digest 的 datasets/models/metrics 列表）：**

**仅保留在本文（论文私有方法与组件，不建独立页面）：**

概念与论文私有方法不建独立页面，跨论文综合由 topic 页承担；实体页由 publish_wiki.py 确定性生成。
