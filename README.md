...
<p align="center"><sub>效果图使用经典 AI 论文标题和替代内容，仅用于说明产品形态，不包含真实 Vault 或私人研究资料。</sub></p>
...
| **基于知识库提问** | 基于知识树与 Agent Tree 定位相关 Paper Card 和 Topic，生成带来源依据的回答、比较或综述 | 直接调用已经积累的研究知识，减少重复翻阅和整理 |
...
论文首先进入独立分析流程，生成经过检查的 Paper Card。相关论文满足准入条件后，框架创建或更新 Topic，并同步维护索引、知识树、Agent Tree 和日志。研究者可以基于现有 Wiki 进行提问、查证和综述检索，也可以进一步挖掘跨主题的研究空白。
...
- Topic 维护多篇论文形成的综合认识。
- 知识树（人读导航）与 Agent Tree（检索第一跳）共同支持提问、检索、查证和综述。
- 开放问题和研究空白随新论文持续更新。
