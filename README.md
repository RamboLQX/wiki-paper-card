...
**`wiki-paper-card` 是一套面向长期研究的论文知识组织框架。** 它将每篇论文整理为带原文定位的 Paper Card，在批量处理中确定性生成可复用实体的页面（公开数据集、基准、模型族与指标），并通过 Topic 页面综合同一研究问题下的方法、结果、边界、共识、分歧与研究空白。随着论文持续加入，Wiki 中的知识页面、关系、索引和研究仪表盘同步更新，为论文回顾、跨论文比较、综述写作和研究选题提供可核验的知识基础。
...
每篇论文首先形成一张完整的 Paper Card，保留研究问题、方法、实验、结论边界、局限和研究想法，并为关键结论记录页码、图、表或公式位置。批量处理随后连接多篇论文中的实体与研究主题：公开数据集、基准、模型族与指标由发布器确定性生成实体页并聚合引用来源，相关论文的方法、结果与分歧则在 Topic 页面累积和比较。用户可以从任意知识页面回到来源论文核验，也可以从已有问题和研究空白继续组织下一批阅读。
...
| **Entity** | 公开数据集、基准、模型族与指标等公共对象的别名和引用来源，由发布器从论文 digest 确定性生成 | 追踪同一公开对象被哪些论文使用，并从任意来源论文回到对象页面核验 |
...
| 论文越攒越多，方法和研究对象散落各处 | Entity 页面确定性聚合公开数据集、基准、模型族与指标的引用来源，Topic 页面在同一问题下对照论文，让同一知识对象保持连续 |
...
实体页由发布器从批次 digest 确定性生成与更新；topic 页基于独立来源支持或已有知识连接创建和更新；尚未形成跨论文价值的内容保留在 Paper Card 中。
...
| Obsidian 路径映射、KB context、批量编排、digest、link plan 与 topic 决策、实体页确定性生成、Wiki 发布与幂等更新 | 本项目 |
...
单篇处理至少会生成 `wiki/sources/` 下的 Paper Card 来源页，发布器还会从 digest 的公开数据集、基准、模型族与指标确定性生成或更新实体 stub。单篇论文不会凭空创建新的 topic。
...
公开数据集、基准、模型族与指标由发布器从论文 digest 确定性生成实体 stub（归一化名称、合并变体、向已有页面追加"引用来源"），不需要也不允许 Agent 决策。topic 只有在至少两篇论文共享同一问题、机制或证据空间，或者新论文回答或挑战已有 topic 的开放问题时才会创建或更新。
...
已有 vault 中的旧 concept 页面不再被发布器写入或更新，也不会出现在知识树中；可手动标记 `archived` 或保留为只读参考。
...
新论文生成带原文定位的 Paper Card，公开数据集、基准、模型族与指标由发布器确定性生成实体 stub，跨论文证据更新 Topic，索引和研究仪表盘同步维护；已有开放问题与研究空白又能组织后续阅读。背后的确定性流水线（prepare → finalize → audit → publish）与逐脚本细节见 [docs/architecture.md](docs/architecture.md)。
...
Paper Card 保留论文的完整细节；实体页是只含别名与引用来源的确定性 stub；topic 页面负责跨论文综合。审计报告和中间文件位于对应批次的工作目录，不进入最终 Wiki。
