...
2. 在知识库上提问、检索、查证或写综述 → 不调用 `wiki-paper-card`，遵循 `skills/wiki-shared/references/retrieval-protocol.md`：先读 `wiki/meta/knowledge-tree.md`（具体论文未命中时回退 `wiki/index.md` 与 `wiki/sources/`；选题类查询同时读 `wiki/meta/research.md`），按 lookup 或 survey 模式检索，结论必须带页面与证据指针。检索与综述只读，不回写 wiki。
